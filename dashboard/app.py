...
class TaskManager:
    ...
    def load_tasks() -> Dict[str, Any]:
        ...
        if not TASKS_FILE.exists():
            return {"tasks": [], "metadata": {"version": "1.0", "updated": datetime.now().isoformat()}}
    ...
    def git_snapshot() -> None:
        ...
        try:
            ...
            subprocess.run([
                'git', 'commit', '-m', 
                f'dashboard: snapshot {datetime.now().isoformat()}'
            ], capture_output=True)
        except Exception:
