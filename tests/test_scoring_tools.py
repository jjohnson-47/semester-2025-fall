...
def test_cost_of_delay_and_freshness_decay() -> None:
    now = datetime(2025, 8, 20, tzinfo=UTC)
...
def test_select_now_queue_fallback_constraints() -> None:
    ...
    cands = [
        Candidate(
            id="A", course="X", score=10.0, est_minutes=30, is_chain_head=True, status="todo"
        ),
        Candidate(id="B", course="Y", score=9.0, est_minutes=70, is_chain_head=True, status="todo"),
        Candidate(
            id="C", course="X", score=8.0, est_minutes=30, is_chain_head=True, status="doing"
        ),
        Candidate(
            id="D", course="Z", score=7.0, est_minutes=30, is_chain_head=False, status="todo"
        ),
    ]
