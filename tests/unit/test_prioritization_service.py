...
class TestPrioritizationProperties:
    ...
    def test_monotonicity_basic_scoring_properties(self, repo, num_tasks):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Create tasks with unique IDs per test run
            import time
            import uuid
            tasks = []
            unique_prefix = str(uuid.uuid4())[:8]  # Unique prefix per test run
            for i in range(num_tasks):
                task = TaskBuilder(f"monotone-{unique_prefix}-{i}").with_course("MATH221").build()
                tasks.append(task)
                repo.db.create_task(task)
    ...
    def test_stability_under_small_changes(self, repo, num_tasks, noise_epsilon):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            unique_prefix = str(uuid.uuid4())[:8]  # Unique prefix per test run
            for i in range(num_tasks):
                task = TaskBuilder(f"stable-{unique_prefix}-{i}").with_course(f"COURSE{i%3}").build()
                tasks.append(task)
                repo.db.create_task(task)
