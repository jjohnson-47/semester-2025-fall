    ...
    def print_status(self):
        ...
        if agents:
            ...
            for agent_id, agent in agents.items():
                ...
                print(f"{icon} {agent_id[:18]:<18} {agent['status']:<12} {agent.get('lane', 'N/A')[:18]:<20} {worktree_short:<30}")
        ...
        if lanes:
            ...
            sorted_lanes = sorted(lanes.items(), 
                                key=lambda x: {'high': 0, 'medium': 1, 'low': 2}.get(x[1].get('priority', 'low'), 3))
...
def main():
    ...
    parser.add_argument('--tracker', default='.orchestration/tracker.json',
                      help='Path to tracker file')
    parser.add_argument('--web', action='store_true',
                      help='Run web-based monitor')
