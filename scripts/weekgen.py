...
class WeeklyGenerator:
    ...
    def generate_weeks(self):
        ...
        for i, week in enumerate(weeks[:16], 1):
            week_name = "Finals_Week" if week.get("is_finals") else f"Week_{i:02d}"
        ...
        for i, week in enumerate(weeks[:16], 1):
            if week.get("is_finals"):
                index.append(
                    f"- [Finals Week]({self.output_dir}/Finals_Week/) - {week['start']} to {week['end']}"
                )
            else:
                week_name = f"Week_{i:02d}"
                holidays = f" - {', '.join(week['holidays'])}" if week["holidays"] else ""
                index.append(
                    f"- [Week {i}]({self.output_dir}/{week_name}/) - {week['start']} to {week['end']}{holidays}"
                )
