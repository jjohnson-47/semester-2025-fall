...
def http_post(url: str, data: Optional[bytes] = None, timeout: float = 5.0) -> tuple[int, bytes, dict[str, str]]:
...
def main() -> int:
  ...
  parser.add_argument("--spawn", action="store_true", help="Start the server for the duration of the test")
  ...
  try:
    ...
    if failures:
      print(f"\n{len(failures)} failure(s): " + ", ".join(f.name for f in failures), file=sys.stderr)
      return 1
    return 0
  finally:
