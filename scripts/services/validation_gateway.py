...
class ValidationGateway:
    ...
    def _validate_dates(self, course: NormalizedCourse, report: ValidationReport) -> None:
        ...
        for week in course.schedule_weeks:
            ...
            if week.assignments and week.assignments.value:
                for assignment in week.assignments.value:
                    if isinstance(assignment, str) and ("Saturday" in assignment or "Sunday" in assignment):
                        weekend_warnings += 1
        ...
        if course.important_dates:
            ...
            missing_dates = [
                ...
                if date not in [k.lower() for k in course.important_dates.keys()]
            ]
    ...
    def _validate_cross_references(self, course: NormalizedCourse, report: ValidationReport) -> None:
    ...
    def _calculate_metrics(self, course: NormalizedCourse, report: ValidationReport) -> dict[str, Any]:
    ...
    def _calculate_completeness(self, course: NormalizedCourse) -> float:
        ...
        if course.schedule_weeks:
            schedule_score = min(len(course.schedule_weeks) / 14, 1.0)
        else:
            schedule_score = 0.0
    ...
    def generate_summary_report(self, reports: dict[str, ValidationReport]) -> dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": sum(r.confidence_score for r in reports.values()) / len(reports) if reports else 0,
